...
use MOM_io,            only : file_exists, field_exists, SIS_read_data=>MOM_read_data
...
use MOM_coms,         only : sum_across_PEs, reproducing_sum, EFP_type
...
subroutine set_ocean_top_fluxes(Ice, IST, IOF, FIA, OSS, G, US, IG, sCS, SMB)
  type(ice_data_type),        intent(inout) :: Ice !< The publicly visible ice data type.
...
!  enddo ; enddo ; enddo
  ...
  do j=jsc,jec ; do i=isc,iec
    i2 = i+i_off ; j2 = j+j_off! Use these to correct for indexing differences.
    Ice%flux_t(i2,j2) = US%QRZ_T_to_W_m2*IOF%flux_sh_ocn_top(i,j)*G%mask2dT(i,j)
    Ice%flux_q(i2,j2) = US%RZ_T_to_kg_m2s*IOF%evap_ocn_top(i,j)*G%mask2dT(i,j)
    Ice%flux_sw_vis_dir(i2,j2) = US%QRZ_T_to_W_m2*IOF%flux_sw_ocn(i,j,VIS_DIR)*G%mask2dT(i,j)
    Ice%flux_sw_vis_dif(i2,j2) = US%QRZ_T_to_W_m2*IOF%flux_sw_ocn(i,j,VIS_DIF)*G%mask2dT(i,j)
    Ice%flux_sw_nir_dir(i2,j2) = US%QRZ_T_to_W_m2*IOF%flux_sw_ocn(i,j,NIR_DIR)*G%mask2dT(i,j)
    Ice%flux_sw_nir_dif(i2,j2) = US%QRZ_T_to_W_m2*IOF%flux_sw_ocn(i,j,NIR_DIF)*G%mask2dT(i,j)
    Ice%flux_lw(i2,j2) = US%QRZ_T_to_W_m2*IOF%flux_lw_ocn_top(i,j)*G%mask2dT(i,j)
    Ice%flux_lh(i2,j2) = US%QRZ_T_to_W_m2*IOF%flux_lh_ocn_top(i,j)*G%mask2dT(i,j)
    Ice%fprec(i2,j2) = US%RZ_T_to_kg_m2s*IOF%fprec_ocn_top(i,j)*G%mask2dT(i,j)
    Ice%lprec(i2,j2) = US%RZ_T_to_kg_m2s*IOF%lprec_ocn_top(i,j)*G%mask2dT(i,j)
    Ice%runoff(i2,j2)  = US%RZ_T_to_kg_m2s*FIA%runoff(i,j)*G%mask2dT(i,j)
    Ice%calving(i2,j2) = US%RZ_T_to_kg_m2s*FIA%calving(i,j)*G%mask2dT(i,j)
    Ice%runoff_hflx(i2,j2)  = US%QRZ_T_to_W_m2*FIA%runoff_hflx(i,j)*G%mask2dT(i,j)
    Ice%calving_hflx(i2,j2) = US%QRZ_T_to_W_m2*FIA%calving_hflx(i,j)*G%mask2dT(i,j)
    Ice%flux_salt(i2,j2) = US%S_to_ppt*US%RZ_T_to_kg_m2s*IOF%flux_salt(i,j)*G%mask2dT(i,j)
...
# include "version_variable.h"
  ...
  character(len=128) :: filename, fieldname, note_header, error_header
  ...
  ! restarts and advection.
    ...
    if (Ice%do_smb_adjustment) then
       ...
       call get_param(param_file, mdl, "SMB_WINDOW", pmt_window, &
                 ...
                 units="days", default=0.0, scale=1.0)
       call get_param(param_file, mdl, "READ_SMB", read_pmt, &
                 "If true, read the time varying smb constraint "//&
                 "from a file. ", &
                 default=.false.)
       ...
       if (.not. read_pmt) then
          SMB(1)%smb_target_fixed=smb_south
          SMB(3)%smb_target_fixed=smb_north
          SMB(2)%smb_target_fixed=0.0 ! land storage residual
       endif
       ...
       if (associated(Smb(k)%smb_hist)) then
          ...
          if ( field_exists(filename, fieldname) ) then
             other_fields_exist = .true.
             write (outunit,*) trim(note_header), ' Reading restart info for ',         &
                  trim(fieldname), ' from ',  trim(filename)
             call SIS_read_data('INPUT/'//filename, fieldname, Smb(k)%smb_hist)
          else if (Time  > Time_init) then
             call SIS_error(WARNING, trim(error_header) // ' Couldn''t find field ' //     &
...
!      if (adjust_smb) then
!        call MOM_read_data(SMB(k)%smb_hist
!      endif
...
subroutine sfc_mass_in_rescale_factor(Ice, Smb, rname)
    ...
    !Smb%total = reproducing_sum(Smb%net_mass_in,EFP_sum=Smb%total_EFP)
    Smb%total_in=sum(Smb%mass_in(is:ie,js:je))
    call sum_across_PEs(Smb%total_in)
    !Smb%total_in = reproducing_sum(Smb%mass_in,EFP_sum=Smb%total_in_EFP)
    Smb%total_out=sum(Smb%mass_out(is:ie,js:je))
    call sum_across_PEs(Smb%total_out)
    !Smb%total_out = reproducing_sum(Smb%mass_out,EFP_sum=Smb%total_out_EFP)
    ...
    if (Smb%read_pmt) then
      call time_interp_external(Smb%id_target, Ice%Time, Smb%smb_target )
    else
  ...
  subroutine close_sfc_mass_balance(Ice,Smb)
    ...
    if (Smb(2)%read_pmt) then
     call time_interp_external(Smb(2)%id_target, Ice%Time, Smb(2)%smb_target )
    else
      Smb(2)%smb_target = Smb(2)%smb_target_fixed
    endif
    ...
    !Smb(2)%total = reproducing_sum(Smb(2)%net_mass_in,EFP_sum=Smb(2)%total_EFP)
    Smb(2)%total_in=sum(Smb(2)%mass_in)
    call sum_across_PEs(Smb(2)%total_in)
    !Smb(2)%total_in = reproducing_sum(Smb(2)%mass_in,EFP_sum=Smb(2)%total_in_EFP)
    Smb(2)%total_out=sum(Smb(2)%mass_out)
    call sum_across_PEs(Smb(2)%total_out)
    !Smb(2)%total_out = reproducing_sum(Smb(2)%mass_out,EFP_sum=Smb(2)%total_out_EFP)
